...
function [updated_state_matrix, transitionTally] = transition5(transition,transition_path, state_matrix, StateMatCols, state_mat_demog_group_idx, future_state_param_names, calibrationConstant)
    ...
    for initialState = 1:size(state_trans_mat,1)
        ...
        % apply calibration constant to scale the probability applied here
            % this is only done for deaths
            % for all others, this value is 1
