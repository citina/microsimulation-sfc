...
%    status_index = 4;
   ...
   for i = 1:num_subgroups
    ...
    %    boolVec = (ageVec >= ageMin & ageVec <= ageMax & raceVec == race & statusVec == status & awareVec == aware & treatVec == treatment & alive == alive);
       boolVec = (ageVec >= ageMin & ageVec <= ageMax & awareVec == aware & statusVec ~= 0 & aliveVec == alive);
       ...
       numDiag = min([numDiag, length(indices)]);

       % randomly sample the number of individuals without replacement
       giveDiag = datasample(indices, numDiag, 'Replace', false);

       newDiag = ones(numDiag, 1);

       % change the indices of the people identified
       state_matrix(giveDiag, StateMatCols.aware) = newDiag;
   ...
   A = state_matrix_copy(:, StateMatCols.aware) ~= state_matrix(:, StateMatCols.aware);
